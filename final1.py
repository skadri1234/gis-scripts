...
plt.text(0.1, 0.9, f'R-squared = {r_value**2:.3f}\nP-value = {p_value:.3f}\n{regression_equation}',
...
for species in species_to_test:
    species_residuals = df[df['Spp'] == species]['Residuals']
    plt.boxplot(species_residuals, labels=[species])
    plt.title(f'Boxplot by Species: {species}')
    plt.show()

    if shapiro_p_value < 0.05:
        # Data not normal, use Wilcoxon rank sum test
        stat, p_value = stats.ranksums(species_residuals, df['Residuals'])
        test_type = 'Wilcoxon Rank Sum'
    else:
        # Data normal, use 2-sample t-test
        stat, p_value = stats.ttest_ind(species_residuals, df['Residuals'])
        test_type = '2-sample t-test'

    print(f'{test_type} for {species}: p-value = {p_value:.3f}')

# Boxplots by species type
for species_type in df['SppType'].unique():
    type_residuals = df[df['SppType'] == species_type]['Residuals']
    plt.boxplot(type_residuals, labels=[species_type])
    plt.title(f'Boxplot by Species Type: {species_type}')
    plt.show()

    if shapiro_p_value < 0.05:
        # Data not normal, use Wilcoxon rank sum test
        stat, p_value = stats.ranksums(type_residuals, df['Residuals'])
        test_type = 'Wilcoxon Rank Sum'
    else:
        # Data normal, use 2-sample t-test
        stat, p_value = stats.ttest_ind(type_residuals, df['Residuals'])
        test_type = '2-sample t-test'

    print(f'{test_type} for {species_type}: p-value = {p_value:.3f}')
...
# Print the pairwise tests table and save to a CSV
